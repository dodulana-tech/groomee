...
export default function AssignProButton({
  bookingId,
  serviceName,
  pros,
}: {
  ...
  async function assign() {
    ...
    try {
      const res = await fetch(`/api/admin/bookings/${bookingId}/assign`, {
        ...
        body: JSON.stringify({ proId: selected }),
      });
  ...
  const available = pros.filter((g) => g.availability === "ONLINE");
  const busy = pros.filter((g) => g.availability === "BUSY");
              ...
              {pros.length === 0 && (
                <div className="text-center py-8">
                  ...
                  <p className="text-sm font-semibold text-gray-700">
                    No available pros for {serviceName}
                  </p>
                  <p className="text-xs text-gray-400 mt-1">
                    No active pros in this zone can perform this service
                    right now
                  </p>
                </div>
              )}
